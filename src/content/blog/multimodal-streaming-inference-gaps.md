...
这篇文章讨论的是：类似 [Qwen3-Omni](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=Qwen3-Omni&zhida_source=entity) 以及一系列 Omni 和 [LPM 1.0](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=LPM+1.0&zhida_source=entity) 这种支持实时流式输出的模型，传统的多模态输出（如图片生成、视频生成模型等）并不在考虑之中。



过去几年，LLM Serving 的主线已经比较清楚：Continuous Batching 解决批处理吞吐，[PD 分离](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=PD+%E5%88%86%E7%A6%BB&zhida_source=entity)把 prefill 和 decode 拆开调度，后面还有 AF 分离、[KV 迁移](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=KV+%E8%BF%81%E7%A7%BB&zhida_source=entity)、prefix cache、算子融合、稀疏注意力和[线性注意力适配](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=%E7%BA%BF%E6%80%A7%E6%B3%A8%E6%84%8F%E5%8A%9B%E9%80%82%E9%85%8D&zhida_source=entity)。这些工作很重要，但它们大多还是围绕一个核心目标展开：在尽可能高的 GPU 利用率下，把更多 request 跑完。

实时流式推理的约束更硬。以 [TTS](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=TTS&zhida_source=entity) 或实时 Omni 对话为例，系统要先保证 [RTF](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=RTF&zhida_source=entity) 小于 1，让用户听到或看到的输出不断流，然后再谈并发、吞吐和成本。它关心的是长时间存活的 session，而不是一次请求的完成时间。
...
图源：LPM 1.0 技术报告 Figure 5（[arXiv:2604.07823](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/2604.07823)）。这里可以看到，`speaking audio` 和 `listening audio` 不是简单拼到输入里，而是分别经过 audio encoder，并在 [DiT block](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=DiT+block&zhida_source=entity) 里通过 `Audio-Video Cross Attention` 注入。
...
图源：LPM 1.0 技术报告 Figure 4（[arXiv:2604.07823](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/2604.07823)）。这张图能说明 LPM 里的 reference 不是单张脸图，而是一组多粒度身份锚点：全局外观、多视角身体参考和表情参考共同约束长时间生成中的身份一致性。
...
SGLang 的重点是 prefix 结构化复用。[RadixCache](https://zhida.zhihu.com/search?content_id=273821134&content_type=Article&match_order=1&q=RadixCache&zhida_source=entity) 用 radix tree 管理 prefix，支持 LRU、LFU、priority、SLRU 等驱逐策略；HiCache 进一步把 KV 扩展到 GPU、host 和外部存储之间。它也有 `SessionAwareCache` 这类机制，可以在 streaming session 之间保留和恢复一部分 KV 状态。
...
图源：LPM 1.0 技术报告 Figure 6（[arXiv:2604.07823](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/2604.07823)）。这张图把 online 路径画得更直接：Backbone 读取 `noisy-history KV cache` 做长期 rollout 稳定，Refiner 读取 `clean-history KV cache` 恢复最终细节，两段都在 `chunk-wise causal mask` 下运行。
...
图源：LPM 1.0 技术报告 Figure 7（[arXiv:2604.07823](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/2604.07823)）。这张图的重点不是单个模型调用，而是把 Interface、Condition 和 Model 三层放在同一条时间线上：用户输入、`speaking/listening audio condition`、Generator/Refiner/VAE 三段执行，以及 `fixed sink + sliding window cache` 都要按 chunk 对齐。
